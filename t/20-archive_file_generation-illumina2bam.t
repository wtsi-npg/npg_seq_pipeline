...
my $jar_path = join q[/], $current, $ENV{CLASSPATH}; 
...
my $intensities_dir = $util->analysis_runfolder_path() . q{/Data/Intensities};
my $pbcal_dir       = $intensities_dir . q{/BaseCalls};

{
  ...
  my $expected_cmd = q{bsub -q srpipeline -R 'select[mem>}.$mem.q{] rusage[mem=}.$mem.q{,nfs_12=4]' -M} . $mem .
    q{ -R 'span[hosts=1]' -n} . $cpu . 
    qq{ -w'done(123) && done(321)' -J 'illumina2bam_1234_8_20090709-123456' -o $pbcal_dir/log/illumina2bam_1234_8_20090709-123456.%J.out /bin/bash -c 'set -o pipefail;java -Xmx1024m -jar $jar_path/Illumina2bam.jar I=$intensities_dir L=8 B=$pbcal_dir RG=1234_8 PU=123456_IL2_1234_8 LIBRARY_NAME="51021" SAMPLE_ALIAS="SRS000147" STUDY_NAME="SRP000031: 1000Genomes Project Pilot 1" OUTPUT=/dev/stdout COMPRESSION_LEVEL=0 | java -Xmx1024m -jar $jar_path/BamIndexDecoder.jar VALIDATION_STRINGENCY=SILENT I=/dev/stdin  BARCODE_FILE=t/data/taglistfile METRICS_FILE=$pbcal_dir/1234_8.bam.tag_decode.metrics MAX_NO_CALLS=6 CONVERT_LOW_QUALITY_TO_NO_CALL=true CREATE_MD5_FILE=false OUTPUT=/dev/stdout};
  $expected_cmd .= qq{| tee >(bamseqchksum > $pbcal_dir/1234_8.post_i2b.seqchksum)};
  $expected_cmd .= q{ >(md5sum -b | tr -d '"'"'"'"'"'"'"'"'\n *\-'"'"'"'"'"'"'"'"' > } . qq{$pbcal_dir/1234_8.bam.md5)};
  $expected_cmd .= qq{ > $pbcal_dir/1234_8.bam'};
