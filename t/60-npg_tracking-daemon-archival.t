...
# Id:            $Id: 60-npg_tracking-daemon-archival.t 18739 2014-11-26 14:28:42Z mg8 $
# $HeadURL: svn+ssh://svn.internal.sanger.ac.uk/repos/svn/new-pipeline-dev/npg-pipeline/trunk/t/60-npg_tracking-daemon-archival.t $
#
...
{
    my $r = npg_tracking::daemon::archival->new();
    isa_ok($r, 'npg_tracking::daemon::archival');
}

{
    ...
    is($r->daemon_name, 'npg_pipeline_archival_runner', 'default daemon name');
    ...
    is($r->ping, q[daemon --running -n npg_pipeline_archival_runner && ((if [ -w /tmp/npg_pipeline_archival_runner.pid ]; then touch -mc /tmp/npg_pipeline_archival_runner.pid; fi) && echo -n 'ok') || echo -n 'not ok'], 'ping command');
    is($r->stop, q[daemon --stop -n npg_pipeline_archival_runner], 'stop command');
}
