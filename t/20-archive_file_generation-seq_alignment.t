...
subtest 'test 6' => sub {
  ...
  my $qc_out = qq{$qc_in/qc};
  my $args = {};
  $args->{10001} = qq{bash -c ' mkdir -p $bc_path/archive/tmp_\$LSB_JOBID/16850_1#1 ; cd $bc_path/archive/tmp_\$LSB_JOBID/16850_1#1 && vtfp.pl -s -keys samtools_executable -vals samtools1 -keys cfgdatadir -vals \$(dirname \$(readlink -f \$(which vtfp.pl)))/../data/vtlib/ -keys aligner_numthreads -vals `npg_pipeline_job_env_to_threads` -keys br_numthreads_val -vals `npg_pipeline_job_env_to_threads --exclude 1 --divide 2` -keys b2c_mt_val -vals `npg_pipeline_job_env_to_threads --exclude 2 --divide 2` -keys indatadir -vals $bc_path/lane1 -keys outdatadir -vals $bc_path/archive/lane1 -keys af_metrics -vals 16850_1#1.bam_alignment_filter_metrics.json -keys rpt -vals 16850_1#1 -keys reference_dict -vals $ref_dir/picard/Pf3D7_v3.fasta.dict -keys reference_genome_fasta -vals $ref_dir/fasta/Pf3D7_v3.fasta -keys phix_reference_genome_fasta -vals $dir/references/PhiX/Illumina/all/fasta/phix-illumina.fa -keys alignment_filter_jar -vals $odir/t/bin/software/solexa/bin/aligners/illumina2bam/Illumina2bam-tools-1.00/AlignmentFilter.jar -keys alignment_reference_genome -vals $ref_dir/bwa0_6/Pf3D7_v3.fasta -keys bwa_executable -vals bwa0_6 -keys alignment_method -vals bwa_mem \$(dirname \$(dirname \$(readlink -f \$(which vtfp.pl))))/data/vtlib/alignment_wtsi_stage2_template.json > run_16850_1#1.json && viv.pl -s -x -v 3 -o viv_16850_1#1.log run_16850_1#1.json } .
