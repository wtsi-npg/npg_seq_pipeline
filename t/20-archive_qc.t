...

{
  ...
  ok( $qc->_can_run('some path', 'some path', 1),    q{lane is indexed - run tag metrics on a lane} );
  ok( !$qc->_can_run('some path', 'some path', 1, 1), q{lane is indexed - do not run tag metrics on a plex} );
  ...
  ok( !$qc->_can_run('some path', 'some path', 1),   q{run is not indexed - do not run tag metrics on a lane} );
  ok( !$qc->_can_run('some path', 'some path', 1, 1), q{run is not indexed - do not run tag metrics on a plex} );
  ...
  SKIP: {
    ...
    ok( !$qc->_can_run('some path', 'some path', 1),  q{lane is indexed - do not run gcbias on a lane} );
    ok( $qc->_can_run('some path', 'some path', 1,1), q{lane is indexed - run gcbias on a plex} );
    ...
    ok( $qc->_can_run('some path', 'some path', 1),   q{run is not indexed - run gcbias on a lane} );
    ok( !$qc->_can_run('some path', 'some path', 1, 1),q{run is not indexed - do not run gcbias on a plex} );
  }
}

{
  ...
  ok ($qc->_can_run('some path', 'some path', 1), 'ref repository not available - genotype check _can_run defaults to true');
  $qc = npg_pipeline::archive::file::qc->new($init);
  ok ($qc->_can_run('some path', 'some path', 1), 'genotype check can run for a non-indexed lane');
  ok (!$qc->_can_run('some path', 'some path', 6), 'genotype check cannot run for an indexed lane');
  ok ($qc->_can_run('some path', 'some path', 6, 0), 'genotype check can run for tag 0 (the only plex is a human sample)');
  ok ($qc->_can_run('some path', 'some path', 6, 1), 'genotype check can run for tag 1 (human sample)');
  ok (!$qc->_can_run('some path', 'some path', 6, 168), 'genotype check cannot run for a spiked phix tag');

  $init->{'qc_to_run'} = 'gc_fraction';
  $qc = npg_pipeline::archive::file::qc->new($init);
  ok ($qc->_can_run($archive_dir, $qc_dir, 6), 'gc_fraction check can run');
  ok ($qc->_can_run($archive_dir, $lane6_qc_dir, 6, 0), 'gc_fraction check can run');
  ok ($qc->_can_run($archive_dir, $lane6_qc_dir, 6, 1), 'gc_fraction check can run');
}
