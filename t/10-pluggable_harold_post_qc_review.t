...

{
  ...
  isa_ok($post_qc_review, q{npg_pipeline::pluggable::harold::post_qc_review});

  is($post_qc_review->id_run, 1234, 'run id set correctly');
  lives_ok { $post_qc_review->main(); } q{no error running main};
  is(join(q[ ], @{$post_qc_review->function_order()}), join(q[ ], @functions_in_order),
  ...
  is($post_qc_review->_update_warehouse_command('warehouse_loader'),
  ...
  is($post_qc_review->_update_warehouse_command('warehouse_loader', 'post_qc_complete'),
  ...
  is($post_qc_review->_update_warehouse_command('npg_runs2mlwarehouse'),
    qq[$prefix  $command], 'update ml_warehouse command');

  $job_name .= '_postqccomplete';
  $prefix = qq[bsub -q lowload -J $job_name ] .
            qq[-o $log_dir_in_outgoing/${job_name}_${timestamp}.out];
  is($post_qc_review->_update_warehouse_command('npg_runs2mlwarehouse', 'post_qc_complete'),
    join(q[ ],$prefix,$preexec,$command),
    'update ml_warehouse command with preexec and change to outgoing');

  $log_dir = $post_qc_review->make_log_dir( $runfolder_path );
  is($post_qc_review->_interop_command(),
    qq[bsub -q lowload -J interop_1234_post_qc_review -R 'rusage[nfs_12=1,seq_irods=15]' -o $log_dir/interop_1234_post_qc_review_] . $timestamp . qq[.out 'irods_interop_loader.pl --id_run 1234 --runfolder_path $runfolder_path'], 'irods_interop_loader.pl command');
}

{
  ...
  $p = npg_pipeline::pluggable::harold::post_qc_review->new(
      runfolder_path => $runfolder_path,
      local          => 1,
  );
