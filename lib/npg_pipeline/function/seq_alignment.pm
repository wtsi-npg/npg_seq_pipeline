...
###
  ...
  return join q( ),
    ...
    q(cd), $working_dir,
    q{ && },
    q{log_label=$(date +%Y%m%d%H%M%S)},
    q{ && },
    q(vtfp.pl),
    q{-template_path $}.q{(dirname $}.q{(readlink -f $}.q{(which vtfp.pl)))/../data/vtlib},
    q(-param_vals), $param_vals_fname,
    q(-export_param_vals), qq(${name_root}).q(_p4s2_pv_out_).qq(${id}).q(_${log_label}.json),
    ...
    qq(> run_${name_root}).q(_${log_label}.json),
    q{&&},
    qq(viv.pl -s -x -v 3 -o viv_${name_root}_).q(${log_label}.log run_).qq(${name_root}_).q(${log_label}.json ),
    q{&&},
