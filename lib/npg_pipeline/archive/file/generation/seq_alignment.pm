...
sub _lsf_alignment_command { ## no critic (Subroutines::ProhibitExcessComplexity)
  ...
  my $do_target_alignment = ($self->_ref($l,q(fasta)) and $l->alignments_in_bam and not ($l->library_type && $l->library_type =~ /Chromium/smx));
  ...
  ########
  my @no_tgt_aln_flags = ();
  if((not $self->_ref($l,q(fasta)) or not $l->alignments_in_bam or ($l->library_type && $l->library_type =~ /Chromium/smx)) and not $nchs and not $spike_tag) {
